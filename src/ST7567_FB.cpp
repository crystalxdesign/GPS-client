...
#define fontuint8_t(x) pgm_read_byte(&cfont.font[x])

#include "hardware/spi.h"

#define CS_IDLE     gpio_put(csPin, 1)
#define CS_ACTIVE   gpio_put(csPin, 0)
#define DC_DATA     gpio_put(dcPin, 1)
#define DC_COMMAND  gpio_put(dcPin, 0)
...
{
  spi_write_blocking(SPI_PORT, &v, 1);
}
// ----------------------------------------------------------------
...
void ST7567_FB::initCmds()
{
  ...
  sendCmd(ST7567_POWER_CTL | 0x4); sleep_ms(50);

  // turn on voltage regulator (VC=1, VR=1, VF=0)
  sendCmd(ST7567_POWER_CTL | 0x6); sleep_ms(50);

  // turn on voltage follower (VC=1, VR=1, VF=1)
  sendCmd(ST7567_POWER_CTL | 0x7); sleep_ms(10);
...
// ----------------------------------------------------------------
const uint8_t ST7567_FB::ditherTab[4*17] PROGMEM = {
...
int ST7567_FB::printChar(int xpos, int ypos, unsigned char c)
{
  ...
  if(xpos+wd+wdL+wdR>SCR_WD) wdR = std::max(SCR_WD-xpos-wdL-wd, 0);
  if(xpos+wd+wdL+wdR>SCR_WD) wd  = std::max(SCR_WD-xpos-wdL, 0);
  if(xpos+wd+wdL+wdR>SCR_WD) wdL = std::max(SCR_WD-xpos, 0);
